...
class AtrialFibrillation(object):
	...
	def _generate_dataset(self, args, device):
		...
		files = glob.glob("../HospitalPrincesaAFDB/*.csv")
		...
		for filename in files:
			...
			for i in range(longitud):
				if labels[i] == 1:
					break
			lastNormalECG = i-1
			tiempo = tt[lastNormalECG]
			for i in range(lastNormalECG+1):
			    if tiempo - tt[i] <= args.tau:
			        break
			ecgs = list(range(i,longitud))
			tt = tt[ecgs]
			tt = np.array(tt)
			tt = tt - tt[0]
			tt = torch.tensor(tt).to(device)
			labels = torch.tensor(np.array(labels[ecgs])).type(torch.float32).to(device)

...
def get_ckpt_model(ckpt_path, model, device):
	...
	# Load checkpoint.
	checkpt = torch.load(ckpt_path,weights_only=False,map_location=torch.device('cuda:0'))
...
# Datos a predecir, los que van a continuación    
	...
	split_dict = {"observed_data": data_dict["data"][:,n_observed_tp,:].clone(),
				...
				"data_to_predict": data_dict["data"][:,n_tp_to_predict,:].clone(),
				"tp_to_predict": data_dict["time_steps"][n_tp_to_predict].clone()}
	...
	if ("mask" in data_dict) and (data_dict["mask"] is not None):
		...
		split_dict["mask_predicted_data"] = data_dict["mask"][:,n_tp_to_predict].clone()

	if ("labels" in data_dict) and (data_dict["labels"] is not None):
		split_dict["labels"] = data_dict["labels"][:,n_tp_to_predict,:].clone()
